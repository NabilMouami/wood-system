...
function CreerFacture() {
  ...
  const handleSubmitBon = (e: React.FormEvent) => {
    ...
    console.log(data2);
    custom_axios
      .post(`/facturation/facture/${userId}/${clientId}`, data2, {
        headers: {
          Authorization: "Bearer " + localStorage.getItem("token"),
        },
      })
      .then((res) => {
        toast.success("Success Facture!!");
      });
  };
  return (
    <div className="bg-[#E6E6FA] font-sans">
      <Typography variant="h1" color="blue-gray" className="text-center">
        Creation Facture Pour Un Client.
      </Typography>
      <form
        ...
        <div ref={componentRef1} className="p-5">
          ...
          <div className="flex justify-between items-centers p-4 m-4 rounded-2xl bg-slate-200">
            ...
            <div>
              ...
              <p>
                <span className="font-bold">Facture Nº:</span>{" "}
                {numFact?.max + 1}
              </p>
            </div>
          </div>
          ...
          <div>
            ...
            <h2 className="flex items-end justify-end text-gray-800 text-xl font-bold">
              Tva{tva * 100}%: {(totalBon * remise * tva).toLocaleString()} Dh.
            </h2>
            <h2 className="flex items-end justify-end text-gray-800 text-xl font-bold">
              Net a payer TTC:{" "}
              {(totalBon * remise * tva + totalBon).toLocaleString()} Dh.
            </h2>
          </div>
        </div>
      </div>
